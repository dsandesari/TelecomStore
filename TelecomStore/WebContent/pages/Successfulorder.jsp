...
<%@ page import="com.tcs.ilp.telecomstore.module3.dao.OrderDAO"%>
<%
int order=(Integer)request.getAttribute("oid");
//int oid=Integer.parseInt(order);
OrderDAO  ob1=new OrderDAO();
float price=ob1.getPrice(order);
%>
<%@ page language="java" %>
<%@ page import="com.tcs.ilp.telecomstore.module3.dao.RetailerDAO" %>
    ...
    RetailerDAO ob2=new RetailerDAO();
    int retailer_id=ob2.getRetailerID(str);
    session.setAttribute("retailer",retailer_id);
    String retname=ob2.getRetailerName(retailer_id);
...
<br>
<h2>Order id is <%=order %></h2><br>
<h2>Total cost is <%=price %></h2>
<br>
<a href="<%=request.getContextPath() %>/pages/Menu.jsp">Back to Menu </a>
           </center>     </div>
                </div> <!-- end of one post -->
